...
const BabysitterForm = (props) => {
  ...
  const onSubmitClick = async (e) => {
    ...
    setworkerLoad(true);
    // API Call
    const {  address , startDate , salary , babyAge , hoursPerDay} = Bookingdetails;
    if (babyAge == 0) {
      props.showAlert("warning", "Please select age of Baby.");
    }
      ...
      const response = await fetch(`${host}/api/worker/BookBabysitter`, {
        ...
        body: JSON.stringify({ workerID : BookWorker.id , name : BookWorker.name , address , salary , startDate , babyAge , hoursPerDay}), // body data type must match "Content-Type" header
      });
      ...
      if (json.status === "SUCCESS") {
        ...
        setBookingDetails({ houseSize:"" , address:"" , startDate:"" , salary:0 , babyAge:0 , hoursPerDay:0});
      }
      else {
        ...
        <div className='container d-flex flex-wrap'>
          <div className="col-xl-6 d-flex">
            <div className="card-body p-md-5 text-black">
              ...
              <div className="form-outline my-3">
                <label className="form-label" htmlFor="babyAge">Age of Baby : </label>
                <input type="Number" id="babyAge" name="babyAge" className="form-control form-control-lg" onChange={onchange} value={Bookingdetails.babyAge} required />
              </div>

              <div className="form-outline my-3">
                <label className="form-label" htmlFor="hoursPerDay">Working Hours per Day : </label>
