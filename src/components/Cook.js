        ...
        <div className='container'>
            ...
            <div className='d-flex justify-content-between align-items-center flex-wrap'>
                    <div className='d-flex justify-content-center align-items-center flex-wrap my-2'>
                        ...
                        <h2 className='mx-5' style={{ color: "#19376D" }}> Cooks : </h2>
                    </div>
                    <div className='d-flex justify-content-center align-items-center flex-wrap my-2'>
                ...
                <div className='d-flex justify-content-center flex-wrap'>
                    {!workerLoad && <h5>{verifiedWorkers.filter(w => w.occupation == "Cook").length === 0 && `No Cooks found.`}</h5>}

                    {/* When no state or city is selected */}
                    {!workerLoad && selectedState == "" && selectedCity=="" && verifiedWorkers.filter(w => w.occupation == "Cook").map((worker) => {
                        return <WorkerCard key={worker._id} vworker={worker}></WorkerCard>;
                    }
                    )}

                    {/* when only state is selected */}
                    {!workerLoad && !selectedState == "" && selectedCity=="" && verifiedWorkers.filter(w => w.occupation == "Cook" && w.state == selectedState).map((worker) => {
                        return <WorkerCard key={worker._id} vworker={worker}></WorkerCard>;
                    }
                    )}

                    {/* When only city is selected */}
                    {!workerLoad && !selectedCity == "" && selectedState=="" && verifiedWorkers.filter(w => w.occupation == "Cook" && w.city == selectedCity).map((worker) => {
                        return <WorkerCard key={worker._id} vworker={worker}></WorkerCard>;
                    }
                    )}

                    {/* When both city and state are selectec */}
                    {!workerLoad && !selectedCity == "" && !selectedState=="" && verifiedWorkers.filter(w => w.occupation == "Cook" && w.city == selectedCity && w.state==selectedState).map((worker) => {
                        return <WorkerCard key={worker._id} vworker={worker}></WorkerCard>;
                    }
                    )}
